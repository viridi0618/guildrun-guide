...
export default function GuidesPage() {
  ...
  const itemList = readyGuides.map((guide, index) => ({ "@type": "ItemList" as const, position: index + 1, item: { "@type": "Article" as const, name: guide.h1, description: guide.description, url: absoluteUrl(`/${guide.slug}`) } }));

  const schemas: Record<string, unknown>[] = [
    { "@context": "https://schema.org", "@type": "CollectionPage", name: "Guildrun Guides", description, url: absoluteUrl("/guides"), dateModified: latestReview, isPartOf: { "@type": "WebSite", name: siteConfig.siteName, url: absoluteUrl("/") } },
  ...
  return <><JsonLd data={schemas} /><div className="shell article"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/">Home</Link><span aria-hidden="true">/</span><span aria-current="page">Guides</span></nav>
    ...
    {Object.entries(groups).map(([groupName, slugs]) => {
      const groupGuides = slugs.map((slug) => readyGuides.find((g) => g.slug === slug)).filter(Boolean);
