# Guildrun Guide — Local Source Log (content-local)
## Date: 2026-07-27
## Version Context: Steam Demo (released 2026-07-16), App ID 3669200

## Source Levels
- **official**: Developer website, Steam store page, developer announcements
- **developer-statement**: Developer FAQ and developer replies on Steam
- **game-store**: Steam store page description
- **community-discussion**: Steam community, Reddit, YouTube
- **editorial**: This site''s analysis and editorial recommendations

## Source List

### L001 — Steam Store Page
- id: L001
- level: game-store
- title: Guildrun on Steam
- url: https://store.steampowered.com/app/3669200/Guildrun/
- publisherOrAuthor: Leyline
- publishedAt: 2026-07-16
- capturedAt: 2026-07-27
- usedFor: [heroes, relics, builds, release-date, difficulty-endless]
- keyFacts:
  - "25 heroes, 180 specializations, 300+ relics and 100+ items in the demo alone"
  - "Progress through 8 difficulties"
  - "Endless mode to chase the top of the global leaderboards"
  - "Weekly Patches and an active Community"

### L002 — Developer FAQ (Pinned Steam Discussion)
- id: L002
- level: developer-statement
- title: FAQ — Guildrun (Developer Post by BrainDeadEdd)
- url: https://steamcommunity.com/app/3669200/discussions/0/570417860284843351/
- publisherOrAuthor: BrainDeadEdd (Leyline developer)
- publishedAt: 2026-07-16
- capturedAt: 2026-07-27
- usedFor: [heroes, relics, builds, release-date, difficulty-endless]
- keyFacts:
  - "We are aiming to release Guildrun in 2027"
  - "The game is compatible on Windows and Mac"
  - "The demo supports English, German, Spanish, Portuguese (Brazil.), Russian, Simplified and Traditional Chinese"
  - "Guildrun is already playable on Steam Deck with mousepad and L1 and L2"
  - "We''re interested in adding controller and official Steam Deck support... but we can''t make any promises"
  - "Yes, we plan for every Steam account to keep the progress they make in the demo in the full game (metaprogression, difficulty unlocks and hero mastery stars)"
  - "Guildrun will be a premium game, and available on Steam for a one-time purchase. We have not landed on a price yet"
  - "a 2-player co-op mode is in development"
  - "PvP is not planned"
  - "25 heroes, 180 specializations, 300+ relics and 100+ items"

- id: L003
- level: community-discussion
- title: Guildrun Steam Community — Build and Strategy Discussions
- url: https://steamcommunity.com/app/3669200/discussions/
- publisherOrAuthor: Various Steam community members
- publishedAt: null
- capturedAt: 2026-07-27
- usedFor: [builds]
- note: Specific community build example referenced in builds.md. Players discuss frontline+backline compositions and hero ranking strategies. All community reports are labeled as community-report and have not been independently reproduced.
